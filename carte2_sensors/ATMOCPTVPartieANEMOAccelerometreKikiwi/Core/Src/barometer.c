/*
 * barometer.c
 *
 *  Created on: 5 oct. 2018
 *      Author: alex
 */
#include "main.h"
#include "barometer.h"
#include "stm32l1xx_hal.h"
#include <math.h>

//TODO : set a proper timing
#define SPI_TIMEOUT 50 //in ms

#define SOUTH_EN HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET);
#define SOUTH_DIS HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET);
#define EAST_EN HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);
#define EAST_DIS HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);
#define WEST_EN HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_RESET);
#define WEST_DIS HAL_GPIO_WritePin(GPIOA, GPIO_PIN_8, GPIO_PIN_SET);

#define CMD_RESET 0x1E
#define CMD_PROM_C1 0xA2
#define CMD_PROM_C2 0xA4
#define CMD_PROM_C3 0xA6
#define CMD_PROM_C4 0xA8
#define CMD_PROM_C5 0xAA
#define CMD_PROM_C6 0xAC

#define PRESSURE_OSR_256  0x40
#define PRESSURE_OSR_512  0x42
#define PRESSURE_OSR_1024 0x44
#define PRESSURE_OSR_2048 0x46
#define PRESSURE_OSR_4096 0x48

#define TEMP_OSR_256      0x50
#define TEMP_OSR_512  	  0x52
#define TEMP_OSR_1024 	  0x54
#define TEMP_OSR_2048     0x56
#define TEMP_OSR_4096     0x58

#define CONVERSION_OSR_256  1
#define CONVERSION_OSR_512  2
#define CONVERSION_OSR_1024 3
#define CONVERSION_OSR_2048 5
#define CONVERSION_OSR_4096 10

uint16_t prom_S[6];
uint16_t prom_E[6];
uint16_t prom_W[6];
extern SPI_HandleTypeDef hspi2;

//min OSR by default
static uint8_t pressAddr = PRESSURE_OSR_256;
static uint8_t tempAddr = TEMP_OSR_256;
static uint32_t convDelay = CONVERSION_OSR_256;

static int32_t temperature;
static int32_t pressure;
static float altitude;

/**
 * @brief init the pressure sensor with default parameters
 */
// Initialisation des capteurs
static void ms5611_init_S();
static void ms5611_init_E();
static void ms5611_init_W();

/**
 * @brief Écrit une commande sur le bus SPI pour chaque capteur
 *
 * @param data La commande à écrire
 */
static void ms5611_write_S(uint8_t data);
static void ms5611_write_E(uint8_t data);
static void ms5611_write_W(uint8_t data);

/**
 * @brief Lit 16 bits du bus SPI pour chaque capteur
 *
 * @param reg Le registre à lire
 * @return La valeur lue sur le bus SPI
 */
static uint16_t ms5611_read16bits_S(uint8_t reg);
static uint16_t ms5611_read16bits_E(uint8_t reg);
static uint16_t ms5611_read16bits_W(uint8_t reg);

/**
 * @brief Lit 24 bits du bus SPI pour chaque capteur
 *
 * @param reg Le registre à lire
 * @return La valeur lue sur le bus SPI
 */
static uint32_t ms5611_read24bits_S(uint8_t reg);
static uint32_t ms5611_read24bits_E(uint8_t reg);
static uint32_t ms5611_read24bits_W(uint8_t reg);

/**
 * @brief Lit les valeurs brutes de température pour chaque capteur
 *
 * @return La valeur brute de température
 */
static uint32_t ms5611_readRawTemp_S();
static uint32_t ms5611_readRawTemp_E();
static uint32_t ms5611_readRawTemp_W();

/**
 * @brief Lit les valeurs brutes de pression pour chaque capteur
 *
 * @return La valeur brute de pression
 */
static uint32_t ms5611_readRawPressure_S();
static uint32_t ms5611_readRawPressure_E();
static uint32_t ms5611_readRawPressure_W();


static void ms5611_init_S()
{
	SOUTH_EN;
	HAL_Delay(10);

	ms5611_write_S(CMD_RESET);
	HAL_Delay(15);

	prom_S[0] = ms5611_read16bits_S(CMD_PROM_C1);
	prom_S[1] = ms5611_read16bits_S(CMD_PROM_C2);
	prom_S[2] = ms5611_read16bits_S(CMD_PROM_C3);
	prom_S[3] = ms5611_read16bits_S(CMD_PROM_C4);
	prom_S[4] = ms5611_read16bits_S(CMD_PROM_C5);
	prom_S[5] = ms5611_read16bits_S(CMD_PROM_C6);
	SOUTH_DIS;
}

static void ms5611_write_S(uint8_t data)
{
	HAL_StatusTypeDef status;
	SOUTH_EN
	status = HAL_SPI_Transmit(&hspi2, &data, 1, SPI_TIMEOUT);
	SOUTH_DIS
}

static uint16_t ms5611_read16bits_S(uint8_t reg)
{
	uint8_t byte[3];
	uint16_t return_value;
	SOUTH_EN
	HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 3, SPI_TIMEOUT);
	SOUTH_DIS
	/**
	 * We dont care about byte[0] because that is what was recorded while
	 * we were sending the first byte of the cmd. Since the baro wasn't sending
	 * actual data at that time (it was listening for command), data[0] will
	 * contain garbage data (probably all 0's).
	 */
	return_value = ((uint16_t)byte[1]<<8) | (byte[2]);
	return return_value;
}

static uint32_t ms5611_read24bits_S(uint8_t reg)
{
	uint8_t byte[4];
	uint32_t return_value;
	SOUTH_EN
	HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 4, SPI_TIMEOUT);
	SOUTH_DIS
	return_value = ((uint32_t)byte[1]<<16) | ((uint32_t)(byte[2]<<8)) | (byte[3]);
	return return_value;
}

static uint32_t ms5611_readRawTemp_S()
{
	uint32_t D2;
	//Convert temp
	ms5611_write_S(tempAddr);
	//Conversion Time
	HAL_Delay(convDelay);
	//Read ADC
	D2 = ms5611_read24bits_S(0x00);

	return D2;
}

static uint32_t ms5611_readRawPressure_S()
{
	uint32_t D1;
	//Convert pressure
	ms5611_write_S(pressAddr);
	//Conversion time
	HAL_Delay(convDelay);
	//Read ADC
	D1 = ms5611_read24bits_S(0x00);

	return D1;
}

void Barometer_init_S()
{
	ms5611_init_S();
}

void Barometer_setOSR(OSR osr)
{
	switch(osr)
	{
		default:
		case OSR_256:
			pressAddr = PRESSURE_OSR_256;
			tempAddr = TEMP_OSR_256;
			convDelay = CONVERSION_OSR_256;
			break;
		case OSR_512:
			pressAddr = PRESSURE_OSR_512;
			tempAddr = TEMP_OSR_512;
			convDelay = CONVERSION_OSR_512;
			break;
		case OSR_1024:
			pressAddr = PRESSURE_OSR_1024;
			tempAddr = TEMP_OSR_1024;
			convDelay = CONVERSION_OSR_1024;
			break;
		case OSR_2048:
			pressAddr = PRESSURE_OSR_2048;
			tempAddr = TEMP_OSR_2048;
			convDelay = CONVERSION_OSR_2048;
			break;
		case OSR_4096:
			pressAddr = PRESSURE_OSR_4096;
			tempAddr = TEMP_OSR_4096;
			convDelay = CONVERSION_OSR_4096;
			break;
	}
}

int32_t Barometer_getTemp_S(bool calculate)
{
	if (calculate)
	{
		Barometer_calculate_S();
	}
	return temperature;
}

int32_t Barometer_getPressure_S(bool calculate)
{
	if (calculate)
	{
		Barometer_calculate_S();
	}
	return pressure;
}

float Barometer_getAltitude_S(bool calculate)
{
	if (calculate)
	{
		Barometer_calculate_S();
	}
	return altitude;
}

void Barometer_calculate_S()
{
	int32_t dT;
	int64_t TEMP, OFF, SENS, P;
	uint32_t D1, D2;
	float press, r, c;

	D1 = ms5611_readRawPressure_S();
	D2 = ms5611_readRawTemp_S();

	dT = D2-((long)prom_S[4]*256);
	TEMP = 2000 + ((int64_t)dT * prom_S[5])/8388608;
	OFF = (int64_t)prom_S[1] * 65536 + ((int64_t)prom_S[3] * dT ) / 128;
	SENS = (int64_t)prom_S[0] * 32768 + ((int64_t)prom_S[2] * dT) / 256;

	if (TEMP < 2000){   // second order temperature compensation
		int64_t T2 = (((int64_t)dT)*dT) >> 31;
		int64_t Aux_64 = (TEMP-2000)*(TEMP-2000);
		int64_t OFF2 = (5*Aux_64)>>1;
		int64_t SENS2 = (5*Aux_64)>>2;
		TEMP = TEMP - T2;
		OFF = OFF - OFF2;
		SENS = SENS - SENS2;
	}

	P = (D1*SENS/2097152 - OFF)/32768;
	temperature = TEMP;
	pressure = P;

	press = (float)pressure;
	r= press/101325.0;
	c = 1.0/5.255;
	altitude = (1 - pow(r,c))*44330.77;
}

// EAST Functions
static void ms5611_init_E()
{
    EAST_EN;
    HAL_Delay(10);

    ms5611_write_E(CMD_RESET);
    HAL_Delay(15);

    prom_E[0] = ms5611_read16bits_E(CMD_PROM_C1);
    prom_E[1] = ms5611_read16bits_E(CMD_PROM_C2);
    prom_E[2] = ms5611_read16bits_E(CMD_PROM_C3);
    prom_E[3] = ms5611_read16bits_E(CMD_PROM_C4);
    prom_E[4] = ms5611_read16bits_E(CMD_PROM_C5);
    prom_E[5] = ms5611_read16bits_E(CMD_PROM_C6);
    EAST_DIS;
}

static void ms5611_write_E(uint8_t data)
{
    HAL_StatusTypeDef status;
    EAST_EN;
    status = HAL_SPI_Transmit(&hspi2, &data, 1, SPI_TIMEOUT);
    EAST_DIS;
}

static uint16_t ms5611_read16bits_E(uint8_t reg)
{
    uint8_t byte[3];
    uint16_t return_value;
    EAST_EN;
    HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 3, SPI_TIMEOUT);
    EAST_DIS;

    return_value = ((uint16_t)byte[1] << 8) | (byte[2]);
    return return_value;
}

static uint32_t ms5611_read24bits_E(uint8_t reg)
{
    uint8_t byte[4];
    uint32_t return_value;
    EAST_EN;
    HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 4, SPI_TIMEOUT);
    EAST_DIS;

    return_value = ((uint32_t)byte[1] << 16) | ((uint32_t)(byte[2] << 8)) | (byte[3]);
    return return_value;
}

static uint32_t ms5611_readRawTemp_E()
{
    uint32_t D2;
    ms5611_write_E(tempAddr);
    HAL_Delay(convDelay);
    D2 = ms5611_read24bits_E(0x00);

    return D2;
}

static uint32_t ms5611_readRawPressure_E()
{
    uint32_t D1;
    ms5611_write_E(pressAddr);
    HAL_Delay(convDelay);
    D1 = ms5611_read24bits_E(0x00);

    return D1;
}
void Barometer_calculate_E()
{
	int32_t dT;
	int64_t TEMP, OFF, SENS, P;
	uint32_t D1, D2;
	float press, r, c;

	D1 = ms5611_readRawPressure_E();
	D2 = ms5611_readRawTemp_E();

	dT = D2-((long)prom_E[4]*256);
	TEMP = 2000 + ((int64_t)dT * prom_E[5])/8388608;
	OFF = (int64_t)prom_E[1] * 65536 + ((int64_t)prom_E[3] * dT ) / 128;
	SENS = (int64_t)prom_E[0] * 32768 + ((int64_t)prom_E[2] * dT) / 256;

	if (TEMP < 2000){   // second order temperature compensation
		int64_t T2 = (((int64_t)dT)*dT) >> 31;
		int64_t Aux_64 = (TEMP-2000)*(TEMP-2000);
		int64_t OFF2 = (5*Aux_64)>>1;
		int64_t SENS2 = (5*Aux_64)>>2;
		TEMP = TEMP - T2;
		OFF = OFF - OFF2;
		SENS = SENS - SENS2;
	}

	P = (D1*SENS/2097152 - OFF)/32768;
	temperature = TEMP;
	pressure = P;

	press = (float)pressure;
	r= press/101325.0;
	c = 1.0/5.255;
	altitude = (1 - pow(r,c))*44330.77;
}
// WEST Functions
static void ms5611_init_W()
{
    WEST_EN;
    HAL_Delay(10);

    ms5611_write_W(CMD_RESET);
    HAL_Delay(15);

    prom_W[0] = ms5611_read16bits_W(CMD_PROM_C1);
    prom_W[1] = ms5611_read16bits_W(CMD_PROM_C2);
    prom_W[2] = ms5611_read16bits_W(CMD_PROM_C3);
    prom_W[3] = ms5611_read16bits_W(CMD_PROM_C4);
    prom_W[4] = ms5611_read16bits_W(CMD_PROM_C5);
    prom_W[5] = ms5611_read16bits_W(CMD_PROM_C6);
    WEST_DIS;
}

static void ms5611_write_W(uint8_t data)
{
    HAL_StatusTypeDef status;
    WEST_EN;
    status = HAL_SPI_Transmit(&hspi2, &data, 1, SPI_TIMEOUT);
    WEST_DIS;
}

static uint16_t ms5611_read16bits_W(uint8_t reg)
{
    uint8_t byte[3];
    uint16_t return_value;
    WEST_EN;
    HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 3, SPI_TIMEOUT);
    WEST_DIS;

    return_value = ((uint16_t)byte[1] << 8) | (byte[2]);
    return return_value;
}

static uint32_t ms5611_read24bits_W(uint8_t reg)
{
    uint8_t byte[4];
    uint32_t return_value;
    WEST_EN;
    HAL_SPI_TransmitReceive(&hspi2, &reg, byte, 4, SPI_TIMEOUT);
    WEST_DIS;

    return_value = ((uint32_t)byte[1] << 16) | ((uint32_t)(byte[2] << 8)) | (byte[3]);
    return return_value;
}

static uint32_t ms5611_readRawTemp_W()
{
    uint32_t D2;
    ms5611_write_W(tempAddr);
    HAL_Delay(convDelay);
    D2 = ms5611_read24bits_W(0x00);

    return D2;
}

static uint32_t ms5611_readRawPressure_W()
{
    uint32_t D1;
    ms5611_write_W(pressAddr);
    HAL_Delay(convDelay);
    D1 = ms5611_read24bits_W(0x00);

    return D1;
}

// Initialisation des capteurs pour EAST et WEST
void Barometer_init_E()
{
    ms5611_init_E();
}

void Barometer_init_W()
{
    ms5611_init_W();
}
void Barometer_calculate_W()
{
	int32_t dT;
	int64_t TEMP, OFF, SENS, P;
	uint32_t D1, D2;
	float press, r, c;

	D1 = ms5611_readRawPressure_W();
	D2 = ms5611_readRawTemp_W();

	dT = D2-((long)prom_W[4]*256);
	TEMP = 2000 + ((int64_t)dT * prom_W[5])/8388608;
	OFF = (int64_t)prom_W[1] * 65536 + ((int64_t)prom_W[3] * dT ) / 128;
	SENS = (int64_t)prom_W[0] * 32768 + ((int64_t)prom_W[2] * dT) / 256;

	if (TEMP < 2000){   // second order temperature compensation
		int64_t T2 = (((int64_t)dT)*dT) >> 31;
		int64_t Aux_64 = (TEMP-2000)*(TEMP-2000);
		int64_t OFF2 = (5*Aux_64)>>1;
		int64_t SENS2 = (5*Aux_64)>>2;
		TEMP = TEMP - T2;
		OFF = OFF - OFF2;
		SENS = SENS - SENS2;
	}

	P = (D1*SENS/2097152 - OFF)/32768;
	temperature = TEMP;
	pressure = P;

	press = (float)pressure;
	r= press/101325.0;
	c = 1.0/5.255;
	altitude = (1 - pow(r,c))*44330.77;
}
// Fonctions pour récupérer température, pression et altitude pour EAST et WEST
int32_t Barometer_getTemp_E(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_E();
    }
    return temperature;
}

int32_t Barometer_getPressure_E(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_E();
    }
    return pressure;
}

float Barometer_getAltitude_E(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_E();
    }
    return altitude;
}

int32_t Barometer_getTemp_W(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_W();
    }
    return temperature;
}

int32_t Barometer_getPressure_W(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_W();
    }
    return pressure;
}

float Barometer_getAltitude_W(bool calculate)
{
    if (calculate)
    {
        Barometer_calculate_W();
    }
    return altitude;
}
